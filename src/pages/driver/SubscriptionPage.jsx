...
const PLAN_BENEFITS = {
  basic: ['Standard listing in rider search', 'Ride-by-ride payouts'],
  pro: ['Everything in Basic', 'Preferred Riders — approve and manage repeat riders'],
  elite: ['Everything in Pro', 'Priority placement in the rider\'s nearby-drivers list'],
}
...
function PlanCard({ plan, isCurrent, currentStatus, highlight, onSubscribe, onRenew }) {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: 18, fontWeight: 700, color: 'var(--color-on-surface)' }}>{label}</p>
    ...
    <div style={{ minHeight: '100dvh', background: 'var(--color-background)', fontFamily: 'var(--font-sans)' }}>
      <header className="sticky top-0 z-40 flex items-center gap-3 px-5 py-3"
        ...
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--color-secondary)' }}>Basic, Pro & Elite driver plans</p>
        </div>
      </header>
          ...
          <div style={{ background: 'white', borderRadius: 14, padding: 16, marginBottom: 20, border: '1px solid rgba(241,245,249,1)' }}>
            <div className="flex items-center justify-between" style={{ marginBottom: 8 }}>
              <p style={{ fontSize: 14, fontWeight: 700, color: 'var(--color-on-surface)' }}>
                {current.subscription_plans?.name?.charAt(0).toUpperCase() + current.subscription_plans?.name?.slice(1)} Plan
              </p>
            ...
            {current.status === 'grace_period' && (
              <p style={{ fontSize: 12, color: '#b45309', marginTop: 8, fontWeight: 600 }}>
                Your plan lapsed on {formatDate(current.expiry_date)}. Renew before the grace period ends to keep Pro/Elite features.
              </p>
            )}
            {current.status === 'expired' && (
              <p style={{ fontSize: 12, color: 'var(--color-error)', marginTop: 8, fontWeight: 600 }}>
                Plan-gated features (Preferred Riders, priority visibility) are now restricted.
              </p>
            )}
          </div>
        )}
                ...
                <div key={h.id} style={{ background: 'white', borderRadius: 12, padding: 12, border: '1px solid rgba(241,245,249,1)' }}>
                  <div className="flex items-center justify-between">
                    <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-on-surface)' }}>
                      {h.subscription_plans?.name?.charAt(0).toUpperCase() + h.subscription_plans?.name?.slice(1)} · ₹{Number(h.subscription_plans?.price ?? 0).toFixed(0)}
                    </p>
